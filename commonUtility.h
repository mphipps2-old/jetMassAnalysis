...
void TH1ScaleByRapidity(TH1* h=0,int etaBin=-1) 
{
  if (etaBin == 0) {
    h->Scale(1./0.6);
  }
...
void TH1ScaleByAcceptance(TH1* h=0,int etaBin=-1) 
{
  double totalAcceptance = 0.;
  double detectorHole = 0.;
  if (etaBin == 0) {
    // |y| < 0.3
    double width = 0.3;
    double height =  3*TMath::Pi()/32;
    detectorHole = width*height;
    totalAcceptance = 2 * width * 2 * TMath::Pi();
    double acceptanceLoss = detectorHole / totalAcceptance;
    double scalingFactor = 1 + acceptanceLoss;
    cout << " etabin0 scaling factor " << scalingFactor << " detectorHole " << detectorHole << " acceptance loss " << acceptanceLoss << endl;
    h->Scale(scalingFactor); // 2.3%
  }
  else if (etaBin == 1) {
    // 0.3 < |y| < 0.8
    double width = 0.5;
    ...
    double acceptanceLoss = detectorHole / totalAcceptance;
    double scalingFactor = 1 + acceptanceLoss;
    cout << " etabin1 scaling factor " << scalingFactor << " detectorHole " << detectorHole << " acceptance loss " << acceptanceLoss << endl;
    h->Scale(scalingFactor); // 2.3%
  }
  else if (etaBin == 2) {
    // 0.8 < |y| < 1.2
    double width = 0.4;
    double height =  3*TMath::Pi()/32;
    detectorHole = (0.2)*height; // hole only runs from 0.8 to 1
    totalAcceptance = 2 * width * 2 * TMath::Pi();
    double acceptanceLoss = detectorHole / totalAcceptance;
    double scalingFactor = 1 + acceptanceLoss;
    cout << " etabin2 scaling factor " << scalingFactor << " detectorHole " << detectorHole << " acceptance loss " << acceptanceLoss << endl;
    h->Scale(scalingFactor); // 1.2%
  }
  else if (etaBin == 3) {
    // no scaling
  }
    ...
    double width = 2.1;
    double height =  3*TMath::Pi()/32;
    detectorHole = (1.0)*height; // hole only runs from [0,1]
    totalAcceptance = 2 * width * 2 * TMath::Pi();
    double acceptanceLoss = detectorHole / totalAcceptance;
    double scalingFactor = 1 + acceptanceLoss;
    cout << " etabin5 scaling factor " << scalingFactor << endl;
...
void ATLASLabel(Double_t x,Double_t y,const char* text, int size=22, float space = 0.16, Color_t color=1)
{
  ...
  tex->DrawLatexNDC(x, y, Form("#scale[1.5]{#font[72]{ATLAS} %s}",text));
