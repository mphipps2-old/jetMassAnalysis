...
int main(int argc, char *argv[]) {
  ...
  TFile *reweightFile = new TFile(Form("ReweightingFactors/ReweightingFactors_kSample%d_DataOverMc_ptCut100Events.root",kSample));
  ...
  for (int eta = 0; eta < nEtaBins; ++eta) {
    for (int cent = 0; cent < nCentBins; ++cent) {
      ...
      rooUnfoldRes_firstHalf[eta][cent] = new RooUnfoldResponse( hMcReco[eta][cent], hMcTruth[eta][cent] );
      rooUnfoldRes_firstHalf[eta][cent]->SetName(Form("responseMatrix_firstHalf_eta%d_cent%d",eta,cent));
    }
  }

  int *****statisticscheck = new int****[3];
  for (int q = 0; q < 3;q++) {
    statisticscheck[q] = new int***[35];
    for (int i = 0; i < nPtBins;i++) {
      statisticscheck[q][i] = new int**[35];	  
      for (int j = 0; j < nMBins;j++) {
	statisticscheck[q][i][j] = new int*[35];
	for (int k = 0; k < nPtBins;k++) {
	  statisticscheck[q][i][j][k] = new int[35];
	}
      }
    }
  }
  ...
  for ( int ijz =2 ; ijz<=4 ; ijz++) {
	...
	if ( tr.jets_ptCalib > ptBins[j] && tr.jets_ptCalib < ptBins[j+1]){binRecoPt = j;}
	if ( tr.jets_genPt > ptBins[j] && tr.jets_genPt < ptBins[j+1]){binTruePt = j;}
      }
      for (int j = 0; j <nMBins;j++) {
	if ( (tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)) > mBins[j] && (tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)) < mBins[j+1]){binRecoM = j;}
	if ( (tr.jets_genMass2/(tr.jets_genPt*tr.jets_genPt)) > mBins[j] && (tr.jets_genMass2/(tr.jets_genPt*tr.jets_genPt)) < mBins[j+1]){binTrueM = j;}
      }
      statisticscheck[ijz-2][binRecoPt][binRecoM][binTruePt][binTrueM] += 1;
    }
  }  
  */
  ...
  for ( int ijz =2 ; ijz<=4 ; ijz++) {
      ...
      for (int j = 0; j <40;j++)
	{
      ...
      if (statisticscheck[ijz-2][binRecoPt][binRecoM][binTruePt][binTrueM] < 20) {
	cout << " cutting bin ijz " << ijz << " recoPt " << binRecoPt << " truePt " << binTruePt << " trueM " << binTrueM << " nevents " << statisticscheck[ijz-2][binRecoPt][binRecoM][binTruePt][binTrueM] << endl;
	continue;}
      */

      //        if (ijz == 2 && tr.jets_ptCalib > 220.) continue;
      if (ijz == 2 && tr.jets_ptCalib > 205.) continue;
	   //    if (ijz == 3 && tr.jets_ptCalib > 505.) continue;
	     if (ijz == 3 && tr.jets_ptCalib > 485.) continue;
	...
	if (i < entries/2) rooUnfoldRes_firstHalf[5][cent]->Fill(tr.jets_ptCalib, (tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)), tr.jets_genPt, (tr.jets_genMass2/(tr.jets_genPt*tr.jets_genPt)), tr.jets_weight * jzNorm * fcalWeight * reweightFactor);
      }
      ...
      if (i < entries/2) rooUnfoldRes_firstHalf[etaBin][cent]->Fill(tr.jets_ptCalib, (tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)), tr.jets_genPt, (tr.jets_genMass2/(tr.jets_genPt*tr.jets_genPt)), tr.jets_weight * jzNorm * fcalWeight * reweightFactor);
    }
  }

  
  // write results to file
  // TFile *fout = new TFile(Form("unfSpectra/rooUnfoldResponse_ReweightingMin0P4Max1P5_sample%d_ptCut10Events.root",kSample),"RECREATE");
  //  TFile *fout = new TFile(Form("unfSpectra/rooUnfoldResponse_ReweightingMin0P4Max1P5_sample%d_ptCut100Events.root",kSample),"RECREATE");
  //TFile *fout = new TFile(Form("unfSpectra/rooUnfoldResponse_ReweightingMin0P4Max1P5_sample%d.root",kSample),"RECREATE");
  //TFile *fout = new TFile(Form("unfSpectra/rooUnfoldResponse_ReweightMin0P2Max2P0_sample%d.root",kSample),"RECREATE");
  //TFile *fout = new TFile(Form("unfSpectra/rooUnfoldResponse_ReweightMin0P4Max1P5_sample%d.root",kSample),"RECREATE");
TFile *fout = new TFile(Form("unfSpectra/rooUnfoldResponse_Reweighting_sample%d_ptCut100Events.root",kSample),"RECREATE");
  
  for (int etaBin = 0; etaBin < nEtaBins; ++etaBin) {
    for (int centBin = 0; centBin < nCentBins; ++centBin) {
      hResMatrix[etaBin][centBin] = (TH2D*) rooUnfoldRes[etaBin][centBin]->Hresponse();  
      hResMatrix_firstHalf[etaBin][centBin] = (TH2D*) rooUnfoldRes_firstHalf[etaBin][centBin]->Hresponse();      
      hResponse_pt[etaBin][centBin]->Write();
      hResponse_m[etaBin][centBin]->Write();
      hResMatrix[etaBin][centBin]->Write();
      hResMatrix_firstHalf[etaBin][centBin]->Write();      
      rooUnfoldRes[etaBin][centBin]->Write(); 
      rooUnfoldRes_firstHalf[etaBin][centBin]->Write(); 
      //      rooUnfoldRes_stats[etaBin][centBin]->Write(); 
    }
  }
