...
int main(int argc, char *argv[]) {
  ...
  for ( int ptBin = 0; ptBin < nPtBins; ++ptBin ) {    
    ...
    hDistJes_pt[ptBin]->Fit("gaus", "","",theMeanJes - .6*theRmsJes, theMeanJes + .6*theRmsJes);
  ...
  for ( int mBin = 0; mBin < nMBins; ++mBin ) {    
    ...
    if (mBin < 10)    hDistJes_m[mBin]->Fit("gaus", "","",1 - 0.9*theRmsJes, 1 + 0.9*theRmsJes);
    else if (mBin >= 10 && mBin < 18)    hDistJes_m[mBin]->Fit("gaus", "","",1 - 1.3*theRmsJes, 1 + 1.3*theRmsJes);
    else hDistJes_m[mBin]->Fit("gaus", "","",0.85 - 1.3*theRmsJes, 0.85 + 1.3*theRmsJes);
