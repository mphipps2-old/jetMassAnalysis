...
int main(int argc, char *argv[]) {
  ...
  for (int etaBin = 0; etaBin < nEtaBins; ++etaBin) {
	...
	hDist[etaBin][ptBin][mBin] = new TH1D(name,name, 100,0,3.5);
  ...
  */
  ...
  for ( int ijz =2 ; ijz<=4 ; ijz++) {
	...
	hDist_eta[5]->Fill ((tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)) / ( tr.jets_genMass2 / (tr.jets_genPt * tr.jets_genPt)), tr.jets_weight * jzNorm * fcalWeight * reweightFactor );
	hDistJes_eta[5]->Fill( tr.jets_ptCalib / tr.jets_genPt , tr.jets_weight * jzNorm * fcalWeight * reweightFactor );

	hDist[5][ptBin][mBin]->Fill ((tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)) / (  tr.jets_genMass2 / (tr.jets_genPt * tr.jets_genPt) ) , tr.jets_weight * jzNorm * fcalWeight * reweightFactor );
	hDistJes[5][ptBin][mBin]->Fill( tr.jets_ptCalib / tr.jets_genPt, tr.jets_weight * jzNorm * fcalWeight * reweightFactor );
      }
      hDist_eta[etaBin]->Fill((tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)) / (  tr.jets_genMass2 / (tr.jets_genPt * tr.jets_genPt) ), tr.jets_weight * jzNorm * fcalWeight * reweightFactor );      
      hDistJes_eta[etaBin]->Fill( tr.jets_ptCalib / tr.jets_genPt , tr.jets_weight * jzNorm * fcalWeight * reweightFactor  );
      
      //      hDist_pt[ptBin]->Fill((tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)) / (  tr.jets_genMass2 / (tr.jets_genPt * tr.jets_genPt) ) , tr.jets_weight * jzNorm * fcalWeight * reweightFactor );
      hDist_pt[ptBin]->Fill((tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)) / (  tr.jets_genMass2 / (tr.jets_genPt * tr.jets_genPt) ),  tr.jets_weight * jzNorm * fcalWeight * reweightFactor );      
	    //      hDistJes_pt[ptBin]->Fill( tr.jets_ptCalib / tr.jets_genPt  , tr.jets_weight * jzNorm * fcalWeight * reweightFactor );
      hDistJes_pt[ptBin]->Fill( tr.jets_ptCalib / tr.jets_genPt,  tr.jets_weight * jzNorm * fcalWeight * reweightFactor );
      
      //      hDist_m[mBin]->Fill((tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)) / (  tr.jets_genMass2 / (tr.jets_genPt * tr.jets_genPt) ) , tr.jets_weight * jzNorm * fcalWeight * reweightFactor );
      hDist_m[mBin]->Fill((tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)) / (  tr.jets_genMass2 / (tr.jets_genPt * tr.jets_genPt) ) );
      //      hDistJes_m[mBin]->Fill( tr.jets_ptCalib / tr.jets_genPt, tr.jets_weight * jzNorm * fcalWeight * reweightFactor  );
      hDistJes_m[mBin]->Fill( tr.jets_ptCalib / tr.jets_genPt);      
      
      hDist[etaBin][ptBin][mBin]->Fill((tr.jets_mass2Calib/(tr.jets_ptCalib*tr.jets_ptCalib)) / (  tr.jets_genMass2 / (tr.jets_genPt * tr.jets_genPt) ) , tr.jets_weight * jzNorm * fcalWeight * reweightFactor );      
      hDistJes[etaBin][ptBin][mBin]->Fill( tr.jets_ptCalib / tr.jets_genPt , tr.jets_weight * jzNorm * fcalWeight * reweightFactor );
      ++jetCount;
    }
  ...
  for ( int etaBin = 0; etaBin < nEtaBins; ++etaBin ) {    
    ...
    hDist_eta[etaBin]->Fit("gaus", "Q","",theMean - 1*theRms, theMean + .5*theRms);
    ...
    hDistJes_eta[etaBin]->Fit("gaus", "Q","",theMeanJes - 1*theRmsJes, theMeanJes + .5*theRmsJes);
  ...
  //  c0->SaveAs(fileName);
  c0->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JMS fit eta"));
  pdf_label = "";
  sprintf(fileName,"Plots/JES/JESFit_eta_sample_%d.png",kSample);
  c_b->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JES fit eta"));
  //  c_b->SaveAs(fileName);
  delete c0;
  delete c_b;
  ...
  for ( int ptBin = 0; ptBin < nPtBins; ++ptBin ) {    
    ...
    hDist_pt[ptBin]->Fit("gaus", "Q","",theMean - 1*theRms, theMean + .5*theRms);
    ...
    hDistJes_pt[ptBin]->Fit("gaus", "Q","",theMeanJes - .6*theRmsJes, theMeanJes + .6*theRmsJes);
  ...
  sprintf(fileName,"Plots/JES/JMSFit_pt_MCWeight_sample%d.png",kSample);
    pdf_label = "";
  c1->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JMS fit pt"));
  //  c1->SaveAs(fileName);
  sprintf(fileName,"Plots/JES/JESFit_pt_MCWeight_sample_%d.png",kSample);
    pdf_label = "";
  c1b->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JES fit pt"));
  //  c1b->SaveAs(fileName);
  ...
  for ( int mBin = 0; mBin < nMBins; ++mBin ) {    
    ...
    if (mBin < 12) { 
      ...
      drawBin(mBins,mBin,"",0.2, 0.7,1,18);
      if (mBin == 0) {
	drawCentrality(kSample,-1,0.70,0.86,1,24);
	ATLASLabel(0.25, 0.92, "Internal",0.085,0.28);//, "Pb+Pb  #sqrt{#font[12]{s_s{NN}}} = 5.02 TeV, 0.49 nb^{-1}", kBlack);
      }
    }
    c2b->cd(mBin+1); 
    double theMeanJes = hDistJes_m[mBin]->GetBinCenter( hDistJes_m[mBin]->GetMaximumBin() );
    double theRmsJes = hDistJes_m[mBin]->GetRMS();
    if (mBin < 10)    hDistJes_m[mBin]->Fit("gaus", "Q","",1 - 0.9*theRmsJes, 1 + 0.9*theRmsJes);
    else if (mBin >= 10 && mBin < 18)    hDistJes_m[mBin]->Fit("gaus", "Q","",1 - 1.3*theRmsJes, 1 + 1.3*theRmsJes);
    else hDistJes_m[mBin]->Fit("gaus", "Q","",0.85 - 1.3*theRmsJes, 0.85 + 1.3*theRmsJes);
  ...
  //  c2->SaveAs(fileName);
    pdf_label = "";
  c2->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JMS fit m"));
  sprintf(fileName,"Plots/JES/JESFit_m_sample_%d.png",kSample);
  //  c2b->SaveAs(fileName);
    pdf_label = "";
  c2b->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JES fit m"));
  ...
  for (int m = 0; m<nMBins; ++m) {
    char cName[256];
    sprintf(cName,"mass%d",m);
    c3[m] = new TCanvas(cName,cName);
    c3[m]->Divide(4,5);
    sprintf(cName,"mass_b%d",m);
    c3_b[m] = new TCanvas(cName,cName);
    c3_b[m]->Divide(4,5);
  }
  */
  ...
  for ( int etaBin = 0; etaBin < nEtaBins; ++etaBin ) {    
    for ( int ptBin  = 0 ; ptBin < nPtBins ; ptBin++) {
      for ( int mBin = 0 ; mBin < nMBins ; mBin++)  {
	//	  hDistJes[etaBin][centBin][ptBin][mBein]->Divide(hPtWidth);
	//        c3[mBin]->cd(ptBin+1); 
	...
	hDist[etaBin][ptBin][mBin]->SetMarkerSize(.2);
	//	  cout << " drawing pt bin " << ptBin << endl;
	hDist[etaBin][ptBin][mBin]->GetXaxis()->SetTitle("(m^{2}/#it{p}_{T}^{2})^{reco} / (m^{2}/#it{p}_{T}^{2})^{truth}");
	//	  	  hDist[etaBin][ptBin][mBin]->GetFunction("gaus")->SetLineColor(2);
	drawBin(ptBins,ptBin+1,"GeV",0.2, 0.7,1,18);
	if (ptBin == 0) {
	  drawCentrality(kSample,-1,0.70,0.86,1,24);
	  ATLASLabel(0.25, 0.92, "Internal",0.085,0.28);//, "Pb+Pb  #sqrt{#font[12]{s_s{NN}}} = 5.02 TeV, 0.49 nb^{-1}", kBlack);
	}
	*/
        hDist[etaBin][ptBin][mBin]->Fit("gaus", "Q","",theMean - 1*theRms, theMean + 1.*theRms);	
	//	else hDist[etaBin][ptBin][mBin]->Fit("gaus", "Q","",1. - 1*theRms, 1. + 1.*theRms);
	if ( hDist[etaBin][ptBin][mBin]->GetFunction("gaus") == NULL || mBin < 5 ) {
	  if (mBin == 4)	  cout << " mBin " << mBin << " setting content to -100 " << endl; 
	...
	hDistJes[etaBin][ptBin][mBin]->Fit("gaus", "Q","",theMeanJes - 1*theRmsJes, theMeanJes + .5*theRmsJes);
  ...
  /*
  for (int m = 0; m<nMBins; ++m) {
    c3[m]->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JMS fit pt_eta_m%d",m));
  }
  */
  ...
  //  c4->SaveAs(fileName);
  c4->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JMS eta"));
  delete c4;
  ...
  //  c5->SaveAs(fileName);
  c5->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JES eta"));
  delete c5;
  ...
  //  c6->SaveAs(fileName);
  c6->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JMS pt"));
  delete c6;
  ...
  //  c7->SaveAs(fileName);
  c7->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JES pt"));
  delete c7;
  ...
  drawCentrality(kSample,-1,0.70,0.86,1,24);
  ATLASLabel(0.25, 0.92, "Internal",0.085,0.28);//, "Pb+Pb  #sqrt{#font[12]{s_{NN}}} = 5.02 TeV, 0.49 nb^{-1}", kBlack);
  TLegend *leg4 = new TLegend(0.5271854,0.6394175,0.9049639,0.8533743,NULL,"brNDC");      
  easyLeg(leg4,"");
  ...
  //  c8->SaveAs(fileName);
  c8->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JMS m"));
  delete c8;
  ...
  //  c9->SaveAs(fileName);
  c9->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JES m"));
  delete c9;
  ...
  for (int etaBin = 0; etaBin < nEtaBins; ++etaBin) {
    ...
    hJMS[etaBin]->SetMaximum(1.8);
    hJMS[etaBin]->SetMinimum(0.6);
    drawCentrality(kSample,-1,0.70,0.86,1,24);
    ATLASLabel(0.40, 0.92, "Internal",0.085,0.2);//, "Pb+Pb  #sqrt{#font[12]{s_{NN}}} = 5.02 TeV, 0.49 nb^{-1}", kBlack);
    sprintf(fileName,"Plots/JES/JMS_MvsPt_etaBin%d_sample%d.png",etaBin,kSample);
    //    c10->SaveAs(fileName);
      pdf_label = "";
  if (etaBin == 5)       c10->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JMS_MvsPt_eta%d",etaBin));
      c10->SaveAs(Form("Plots/JES/sample%d_etaBin%d_JMS2d.root",kSample,etaBin));
    delete c10;
    ...
    hJMR[etaBin]->SetMaximum(1.);
    hJMR[etaBin]->SetMinimum(0.);
    drawCentrality(kSample,-1,0.70,0.86,1,24);
    ATLASLabel(0.40, 0.92, "Internal",0.085,0.2);//, "Pb+Pb  #sqrt{#font[12]{s_{NN}}} = 5.02 TeV, 0.49 nb^{-1}", kBlack);
    sprintf(fileName,"Plots/JES/JMR_MvsPt_etaBin%d_sample%d.png",etaBin,kSample);
    //    c11->SaveAs(fileName);
      pdf_label = "";
  if (etaBin == 5)       c11->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JMR_MvsPt_eta%d",etaBin));
      c11->SaveAs(Form("Plots/JES/sample%d_etaBin%d_JMR2d.root",kSample,etaBin));
    delete c11;
    ...
    hJES[etaBin]->SetMaximum(1.2);
    hJES[etaBin]->SetMinimum(0.8);
    drawCentrality(kSample,-1,0.70,0.86,1,24);
    ATLASLabel(0.4, 0.92, "Internal",0.085,0.2);//, "Pb+Pb  #sqrt{#font[12]{s_{NN}}} = 5.02 TeV, 0.49 nb^{-1}", kBlack);
    sprintf(fileName,"Plots/JES/JES_MvsPt_etaBin%d_sample%d.png",etaBin,kSample);
      pdf_label = "";
      if (etaBin == 5) c12->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JES_MvsPt_eta%d",etaBin));
    //    c12->SaveAs(fileName);
      c12->SaveAs(Form("Plots/JES/sample%d_etaBin%d_JES2d.root",kSample,etaBin));
    delete c12;
    ...
    drawCentrality(kSample,-1,0.70,0.86,1,24);
    ATLASLabel(0.4, 0.92, "Internal",0.085,0.2);//, "Pb+Pb  #sqrt{#font[12]{s_{NN}}} = 5.02 TeV, 0.49 nb^{-1}", kBlack);
    //    sprintf(fileName,"Plots/JES/JER_MvsPt_etaBin%d_sample%d.png",etaBin,kSample);
    //    c13->SaveAs(fileName);
    if (etaBin == 5)    pdf_label = ")";
    if (etaBin == 5) c13->Print(Form("Plots/JES/JES_Plots_sample%d_etaBin%d.pdf%s",kSample,5, pdf_label.c_str()), Form("Title:JER_MvsPt_eta%d",etaBin));
    c13->SaveAs(Form("Plots/JES/sample%d_etaBin%d_JER2d.root",kSample,etaBin));
    delete c13;
  }
  return 0;
}
